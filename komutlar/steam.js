...
exports.run = (client, message, args) => {
    ...
    if (!türkbot) return message.reply('Lütfen Steamde Olan Bir Oyunun Adını Yazın. Örnek: t+steam `[Oyun Adı]`')
    provider.search(türkbot).then(result => {
    ...
    .addField('Fiyati', `Nolmal Fiyat **${results.priceData.initialPrice}** TL İndirimli Fiyat **${results.priceData.finalPrice}** TL`, true)
    ...
    message.channel.send(embed).catch(e => {
        console.log(e)
        message.reply('Hata Olustu Yada `' + türkbot + '` Adlı Oyun Bulunamadı')
    })
})
})
}
